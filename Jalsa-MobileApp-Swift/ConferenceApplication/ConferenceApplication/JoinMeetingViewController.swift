        ...
        postRequest(completion: { data, error in
            ...
            if let data = data {
                ...
                DispatchQueue.main.async {
                    ...
                    guard let ConferenceRoomViewController = meetingView.instantiateViewController(withIdentifier: "conferencemain")
                        as? ConferenceRoomViewController else {
                        self.logger.error(msg: "Unable to instantitateViewController")
                        return
                    }
                    ConferenceRoomViewController.modalPresentationStyle = .fullScreen
                    ConferenceRoomViewController.meetingSessionConfig = meetingSessionConfig
                    ConferenceRoomViewController.meetingId = self.meetingID
                    ConferenceRoomViewController.selfName = self.name
                    self.present(ConferenceRoomViewController, animated: true, completion: nil)
                }
            }
        })
    }
