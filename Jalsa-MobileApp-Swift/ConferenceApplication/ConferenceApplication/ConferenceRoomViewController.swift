...
//
//  Created by Hassan Aljanahi on 6/1/20.
...
class ConferenceRoomViewController: UIViewController {
 ...
 var currentMeetingSession: MeetingSession?
    ...
    @IBOutlet var sendMessages: UITextField!
    ...
    public var meetingSessionConfig: MeetingSessionConfiguration?
      ...
      private let logger = ConsoleLogger(name: "ViewController")
    ...
    override func viewDidLoad() {
        ...
        self.messages.backgroundColor = .clear
       
        let numRows = self.messages.numberOfRows(inSection: 0)
           var contentInsetTop = self.messages.bounds.size.height
           for i in 0..<numRows {
               let rowRect = self.messages.rectForRow(at: IndexPath(item: i, section: 0))
               contentInsetTop -= rowRect.size.height
               if contentInsetTop <= 0 {
                   contentInsetTop = 0
               }
           }
           self.messages.contentInset = UIEdgeInsets(top: contentInsetTop,left: 0,bottom: 0,right: 0)
        self.messages.separatorStyle = .none
        let indexPath = NSIndexPath(row: attendees.count - 1, section: 0)
         if attendees.count != 0 {
        self.messages.scrollToRow(at: indexPath as IndexPath, at: .bottom, animated: true)
        }
   ...
   gradientlayer.frame = messages.bounds
          self.messages.layer.mask = gradientlayer
...
*/
        ...
        func switchCameraClicked() {
            ...
            if let tileState = self.videoTileStatesForDisplay[0]{
                ...
                if let selfVideoTileView = screenView as? DefaultVideoRenderView {
    ...
    extension ConferenceRoomViewController: AudioVideoObserver {
    ...
    extension ConferenceRoomViewController: RealtimeObserver {
    ...
    extension ConferenceRoomViewController: MetricsObserver {
    ...
    extension ConferenceRoomViewController: DeviceChangeObserver {
    ...
    extension ConferenceRoomViewController: VideoTileObserver {
    ...
    extension ConferenceRoomViewController: ActiveSpeakerObserver {
    ...
    extension ConferenceRoomViewController: UITableViewDelegate, UITableViewDataSource {
        ...
        func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
            ...
            let currentAttendee = self.attendees[indexPath.row]
            cell.textLabel?.text = currentAttendee.attendeeName
    ...
    extension ViewController: UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
        ...
        func collectionView(
            ...
            guard let cell = collectionView.dequeueReusableCell(
                withReuseIdentifier: self.videoTileCellReuseIdentifier, for: indexPath)
